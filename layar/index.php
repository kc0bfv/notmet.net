...
try {
	...
	$request["pageKey"] = ($request["pageKey"]<=49) ? (int) $request["pageKey"] : 49;
	...
	$sqlStmt = $db->prepare( "SELECT my_id, title, attribution, lat, lon, alt, imageURL, line2, line3, line4, type, dimension, usgs_id, usgs_name, usgs_class, usgs_state, usgs_county FROM POI_Table USE INDEX (lonlatclass) WHERE lat>=:latmin AND lat<=:latmax AND lon>=:lonmin AND lon<=:lonmax $classFilter ORDER BY my_id ASC LIMIT 0, 200" );
...
//	exit;
	...
	$response["errorString"] = "Database error.  Sorry.  I'll fix it ASAP.";
	if( isset( $erroremail ) ) {
		error_log( "Explore Outdoors Error:\n" . "Database error: " . $e->getMessage(), 1, $erroremail );
	}
} catch( Exception $e ) {
	$response["layer"] = $defaultLayerName;
	$response["errorCode"] = 29;
	$response["errorString"] = "Generic error.  Sorry.  I'll fix it ASAP.";
	if( isset( $erroremail ) ) {
		error_log( "Explore Outdoors Error:\n" . "Generic error: " . $e->getMessage(), 1, $erroremail );
	}
}
